...
public class QrCodeHandler {
  ...
  private void showFingerprintOrQrSuccess(
    ...
    builder.setPositiveButton(
        ...
        (dialogInterface, i) -> {
          ...
          String draftText =
              qrParsed.getText1Meaning() == DcLot.DC_TEXT1_DRAFT ? qrParsed.getText1() : null;
          activity.startActivity(AppNav.chatIntent(activity, chatId, -1, draftText));
        });
    builder.setNegativeButton(android.R.string.cancel, null);
  }
  ...
  public void secureJoinByQr(String qrRawString, SecurejoinSource source, SecurejoinUiPath uipath) {
    try {
      ...
      activity.startActivity(AppNav.chatIntent(activity, newChatId));
    } catch (Exception e) {
      e.printStackTrace();
