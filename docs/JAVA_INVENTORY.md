...
**Dead-resource sweep 2026-07-07 (−324 files):** Removed unreferenced `res/` dead weight left over from deleted screens/editors: ~50 layouts (scribble/imageeditor, old conversation-list/profile/welcome/media-view/call/registration UIs), 15 menus (old conversation/profile/registration menus), and ~259 drawables (image-editor icons, never-shipped call UI, old message-bubble/delivery-status/reaction/attachment-picker assets). Build is the safety net — any resource still referenced via `R.`/`@drawable` fails compilation. KEPT (still compile-referenced by the not-yet-removed legacy conversation-input Java island — `InputPanel`/`AttachmentManager`/`MicrophoneRecorderView`/`MediaKeyboard`): `conversation_activity[_attachment_editor_stub|_emojidrawer_stub]`, `conversation_input_panel`, `microphone_recorder_view`, `recording_layout` + their drawables. Retiring that island (blocked on the single live `AttachmentManager.selectLocation` tie) is a code-first follow-up that will free those too. **Value resources:** Android Lint `UnusedResources` reports **~628** unused value entries — **476 strings** (localized across ~30 `values-*/` files), 58 colors, 34 dimens, 17 arrays, 14 plurals, 13 styles, 1 bool/int, + `intro1` (removed). These are low-byte clutter; the localized strings are best removed via the IDE "Remove Unused Resources" refactor (handles all locales) rather than a hand-edit. Note: a few lint-flagged drawables/layouts (`conversation_activity_*stub`, `ic_chevron_up`, `ic_mic_white_48dp`, `slidearrow`, `touch_highlight_background`, `message_bubble_background_sent_alone_with_border`, `button_background`) are intentionally KEPT — still compile-referenced by the legacy conversation-input Java island / kept styles.
